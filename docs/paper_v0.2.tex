...
\author[1]{Andres Espin\thanks{Corresponding author: aaespin3@espe.edu.ec}}
\author[1]{(Junior Developer - Independent Research)}
...
\begin{abstract}
We present \textbf{OrbitalSkyShield}, an open-source Python package for detecting satellite streaks in astronomical images and estimating the Orbital Diffuse Contribution (ODC) to sky brightness. Unlike existing tools, OrbitalSkyShield implements a physically-motivated model of natural sky brightness (Krisciunas \& Schaefer 1991) to separate satellite contamination from lunar and atmospheric contributions. We validate our adaptive detection algorithm on a labeled dataset of 333 astronomical images, achieving 82.77\% precision. We also implement a physical ODC model that accounts for lunar phase, atmospheric scattering, and observational geometry, designed for use with FITS data. The tool is designed for integration into observatory pipelines and provides reproducible, scientifically rigorous measurements of satellite impact on astronomical observations.

\textbf{Keywords:} satellite streaks, light pollution, sky brightness, astronomical instrumentation, image processing

\vspace{1cm}
\textbf{Author Note:} This work is an independent research study developed for educational and technical problem-solving purposes. It does not constitute a peer-reviewed academic publication.
\end{abstract}
...
Our \textbf{AdaptiveDetector} (v0.2) utilizes a purely statistical Image Processing approach, distinct from Machine Learning methods. It processes each image independently through a three-stage pipeline:
\begin{enumerate}
    \item \textbf{Input Processing:} Accepts single-channel astronomical images (FITS or JPEG).
    \item \textbf{Adaptive Thresholding:} Calculates the statistical percentile (typically $p=95-97$th) of the image histograms to generate a binary mask of the brightest pixels, effectively ignoring the background noise floor.
    \item \textbf{Morphological Filtering:} Performs connected component analysis to group adjacent pixels into potential objects.
    \item \textbf{Geometric Validation:} Filters these objects based on their properties. We enforce a minimum Aspect Ratio ($\ge 3:1$) to distinguish linear satellite streaks from circular stars or random noise.
\end{enumerate}
...
The physical model components (defined in Methods) are implemented based on the standard equations from \citet{Krisciunas1991}. \textbf{Note on Validation:} Empirical validation of the ODC module was not performed in this study because the test dataset consists of compressed JPEG images lacking the necessary FITS headers (observation date, coordinates) required to model the natural sky background. Consequently, the ODC results presented are theoretical capabilities of the software when provided with correct FITS data, rather than empirically measured values from the test set.
...
Future work (v0.3+) will focus on developing Deep Learning models (e.g., U-Net or YOLO) trained on this validated dataset. The statistical baseline established in this work will serve as a benchmark to evaluate the performance of these future AI-based detectors, particularly in improving recall for faint streaks which are currently missed by the adaptive thresholding.

\section*{Data Availability}
The code is available at \url{https://github.com/kiboki1234/ProyectoAstronomia}. The validation dataset and results are included in the repository.
